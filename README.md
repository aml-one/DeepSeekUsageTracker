...
A lightweight **240×140 px** WPF widget that sits in the corner of your screen and periodically checks your DeepSeek API balance using the official [`/user/balance`](https://api-docs.deepseek.com/api/get-user-balance) endpoint. No bloat, no browser tab — just your remaining balance, always visible.

### Features

- **Live balance** — calls the DeepSeek API directly with your key
...
- **Dark theme** — custom-styled controls, no Windows chrome
- **Settings popup** — click the ⚙ gear to paste your API key and set the interval
...
> Balance display with green dot (Available), countdown timer, and settings gear.
> Dot turns red on connection errors or invalid key.
...
| Refresh now | Double-click the balance number |
